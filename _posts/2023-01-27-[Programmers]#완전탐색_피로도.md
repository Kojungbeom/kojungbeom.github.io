...
완전탐색 - Brute Force, DFS, 순열, 재귀

### 1. 순열 사용

순열을 활용해서 던전 섞기 -> 섞인 던전 경우로 가능한지 파악 ?? -> 그리고 가능한 경우만 count -> return count
...
1.퍼뮤테이션으로 dungeon case 출력

2. 이중 for문으로 for i in range(len(list(d)))

3. if dugeon[i][0] < k -> 1번 던전부터 최소 피로도 만족하면

4. k -= dugeon[i][1]

5. 만족 못 하면 break

6. 2중 for문 다 만족하면 cnt += 1

7. 다 빠져나와서 return cnt

### 2. DFS

- 깊이 우선 탐색 ; 던전을 차례로 탐색하는 것이기 때문 더 직관적
- 구현 방법: 재귀함수, 스택/큐
- 시간복잡도: O(N)
